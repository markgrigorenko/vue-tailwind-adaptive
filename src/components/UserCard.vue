<template>
  ...
  >
    ...
    <p class="text-sm"><strong>Телефон:</strong> {{ user.phone }}</p>
    <p class="text-sm">
      <strong>Сайт:</strong>
      <a :href="'http://' + user.website" class="text-blue-500 hover:underline">
        {{ user.website }}
      </a>
    </p>
    <button
      @click="$emit('show-photos', user.id, user.name)"
